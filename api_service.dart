...
class ApiService {
  ...
  static Future<List<dynamic>> getRequisitosDoBadge(int badgeId) async {
    final response = await http.get(
  ...
  static Future<void> submeterCandidaturaComImagens({
    required int badgeId,
    required Map<int, XFile?> ficheirosPorRequisito,
  }) async {
    final userId = Session.id;
    if (userId == 0) {
      throw Exception('Sessão inválida. Faz login novamente.');
    ...
    for (final entry in ficheirosPorRequisito.entries) {
      final int idRequisito = entry.key;
      final XFile? ficheiro = entry.value;

      if (ficheiro == null) continue;

      request.fields['requisito_id_$index'] = idRequisito.toString();
    ...
    if (_isJson(body)) {
      ...
      throw Exception(
        data['error']?.toString() ??
            data['message']?.toString() ??
            'Erro ao submeter candidatura',
      );
    }

    throw Exception('Erro ao submeter candidatura');
  }
