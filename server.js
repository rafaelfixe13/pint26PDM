...
// GET /utilizadores/ranking — devolve utilizadores ordenados por pontos
app.get("/utilizadores/ranking", async (req, res) => {
  try {
    const result = await pool.query("SELECT * FROM utilizadores ORDER BY pontos DESC");
    res.json(result.rows);
  } catch (err) {
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO utilizadores
        (nome, email, passwordhash, idrole, emailconfirmado, primeirologin, estadoconta, datacriacao)
       VALUES ($1, $2, $3, 1, FALSE, TRUE, 'ATIVA', NOW())
       RETURNING idutilizador, nome, email, datacriacao`,
      [nome, email, hash]
    );
